...
public readonly struct SeqSpan(int from, int to) : IEquatable<SeqSpan>, IComparable<SeqSpan>
{
    public int From { get; } = Math.Min(from, to);

    public int To { get; } = Math.Max(from, to);
