...
public abstract class BlastPage : TabPage
{
    ...
    protected BlastPage(MainViewModel owner) : base(owner)
    {
        State = PageState.New;
    ...
    public bool CanRun(object? parameter) => !running && !string.IsNullOrWhiteSpace(Query) && File.Exists(ExePath);
    ...
    public bool CanViewMoreHits(object? parameter) => Hits != null && Hits.Count < TotalHits;
    ...
    public void PreviousHit(object? parameter) => SelectedHit = Hits![SelectedHit!.Index - 2];

    public bool CanPreviousHit(object? parameter)
        => Hits != null && SelectedHit != null && SelectedHit.Index >= 2 && SelectedHit.Index <= Hits.Count;

    public void NextHit(object? parameter) => SelectedHit = Hits![SelectedHit!.Index];

    public bool CanNextHit(object? parameter)
        => Hits != null && SelectedHit != null && SelectedHit.Index >= 1 && SelectedHit.Index <= Hits.Count - 1;

    public void PreviousSegmentPair(object? parameter) => SelectedSegment = SelectedHit!.Segments[SelectedSegment!.Index - 2];

    public bool CanPreviousSegmentPair(object? parameter)
        => SelectedHit != null && SelectedSegment != null && SelectedSegment.Index >= 2 && SelectedSegment.Index <= SelectedHit.Segments.Count;

    public void NextSegmentPair(object? parameter) => SelectedSegment = SelectedHit!.Segments[SelectedSegment!.Index];

    public bool CanNextSegmentPair(object? parameter)
        => SelectedHit != null && SelectedSegment != null && SelectedSegment.Index >= 1 && SelectedSegment.Index <= SelectedHit.Segments.Count - 1;
