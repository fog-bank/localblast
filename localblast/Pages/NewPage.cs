...
public class NewPage : TabPage
{
    ...
    public NewPage(MainViewModel owner) : base(owner)
    {
        Header = "New job";

        BrowseBlastBinDirCommand = new(BrowseBlastBinDir);
        BrowseWorkingDirCommand = new(BrowseWorkingDir);

        OpenMakeBlastDbCommand = new(OpenMakeBlastDb, CanOpenMakeBlastDb);

        BrowseBlastnDbCommand = new(BrowseBlastnDb);
        OpenBlastnCommand = new(OpenBlastn, CanOpenBlastn);
        OpenAlginBlastnCommand = new(OpenAlignBlastn, CanOpenBlastn);

        BrowseBlastpDbCommand = new(BrowseBlastpDb);
        OpenBlastpCommand = new(OpenBlastp, CanOpenBlastp);
        OpenAlginBlastpCommand = new(OpenAlignBlastp, CanOpenBlastp);

        BrowseBlastxDbCommand = new(BrowseBlastxDb);
        OpenBlastxCommand = new(OpenBlastx, CanOpenBlastx);
        OpenAlginBlastxCommand = new(OpenAlignBlastx, CanOpenBlastx);

        BrowseTblastnDbCommand = new(BrowseTblastnDb);
        OpenTblastnCommand = new(OpenTblastn, CanOpenTblastn);
        OpenAlginTblastnCommand = new(OpenAlignTblastn, CanOpenTblastn);

        CloseCommand = new(null, static _ => false);
    }
    ...
    public void BrowseBlastBinDir(object? parameter)
    {
        ...
        {
            Owner.BlastBinDir = dlg.FolderName;
    ...
    public void BrowseWorkingDir(object? parameter)
    {
        ...
        if (dlg.ShowDialog() == true)
            Owner.WorkingDirectory = dlg.FolderName;
    }
    ...
    public bool CanOpenBlastn(object? parameter)
        => File.Exists(Path.Combine(Owner.BlastBinDir, "blastn.exe")) && File.Exists(BlastnDbPath);
    ...
    public bool CanOpenBlastp(object? parameter)
        => File.Exists(Path.Combine(Owner.BlastBinDir, "blastp.exe")) && File.Exists(BlastpDbPath);
    ...
    public bool CanOpenBlastx(object? parameter)
        => File.Exists(Path.Combine(Owner.BlastBinDir, "blastx.exe")) && File.Exists(BlastxDbPath);
    ...
    public bool CanOpenTblastn(object? parameter)
        => File.Exists(Path.Combine(Owner.BlastBinDir, "tblastn.exe")) && File.Exists(TblastnDbPath);
